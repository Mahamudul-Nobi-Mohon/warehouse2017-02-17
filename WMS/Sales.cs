    ...
    public partial class Sales : Form
    {
        ...
        private void treeViewPurchaseItemSales_AfterSelect(object sender, TreeViewEventArgs e)
        {
            try
            {
                ...
                {
                    textBoxPrice.Text = reader1211["SalePrice"].ToString();
        ...
        private void SeeTotal_Component()
        {
            ...
            textBoxDisTotal.Text = Math.Ceiling(total_dis).ToString();
        ...
        private void buttonAdd_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (textBoxDiscountTaka.Enabled == true)
                {
                    if (textBoxDiscountTaka.Text == "")
                    {
                        textBoxDiscountTaka.Text = "0";
                    }
                    if (Convert.ToDouble(textBoxDiscountTaka.Text) > temp_sale_price * temp_quantity - 1.00)
                    {
                        MessageBox.Show("Discount Amount Must Be Less Than Price!!");
                        return;
                    }
                    else
                    {
                        temp_discount = Math.Ceiling(Convert.ToDouble(textBoxDiscountTaka.Text));
                    }



                }
                if (textBoxDiscountTaka.Enabled == false)
                {
                    if (textBoxDiscountPercent.Text == "")
                    {
                        textBoxDiscountPercent.Text = "0";
                    }
                    if (Convert.ToDouble(textBoxDiscountPercent.Text) > 99.00)
                    {
                        MessageBox.Show("Discount Persent Must Be Less Than 99.00");
                        return;
                    }
                    else
                    {
                        temp_discount = Math.Ceiling((temp_quantity * temp_sale_price) * (Convert.ToDouble(textBoxDiscountPercent.Text) / 100.00));
                    }
                }
                ...
                double temp_sales_with_vat = temp_sale_total + Math.Ceiling((temp_quantity * temp_sale_price) * vat / 100.00);
                if (alredy > 0.0)
                {
                ...
                {
                    TempSales(temp_product_code,temp_name, temp_quantity, temp_sale_price, temp_purchase_price, temp_invoice_no, temp_discount, vat, temp_sales_with_vat, temp_sale_total, temp_pro_id);
                }
                SeeTotal_Component();
                ...
                textBoxProductName.Text = textBoxPdoductCode.Text = textBoxPrice.Text = textBoxUnitType.Text = textBoxProCate.Text = "";
                textBoxQuantity.Text = textBoxProductSearch.Text = textBoxCurrentStock.Text = "";

                textBoxNetTotal.Text = ((int)Math.Ceiling((Convert.ToDouble(textBoxTotalWithVat.Text) - (Convert.ToDouble(textBoxDisTotal.Text))))).ToString();
       ...
       int PaymentType = 0;
        ...
        private void AddReceivedChangeInTempSales()
        {
            SqlConnection con1 = new SqlConnection(conStr);
            con1.Open();
           // string sql1 = @"UPDATE TempSales SET TempSalesReceivedAmount ='"+textBoxReceiveAmount.Text+"',TempSalesChangeAmount = '"+ textBoxReturnAmount.Text+ "',TempSalesVatTotal = '"+ textBoxNetTotal.Text+ "', TempSalesRemarks = '"+ textBoxRemarks .Text + "' WHERE TempSalesCompanyID = '"+CompanyID+"'";
            string sql1 = @"UPDATE TempSales SET TempSalesReceivedAmount ='"+textBoxReceiveAmount.Text+"',TempSalesChangeAmount = '"+ textBoxReturnAmount.Text+ "', TempSalesRemarks = '"+ textBoxRemarks .Text + "' WHERE TempSalesCompanyID = '"+CompanyID+"'";      // Change
            SqlCommand cmd1 = new SqlCommand(sql1, con1);
            cmd1.ExecuteNonQuery();
            con1.Close();
        }
        ...
        private void buttonReports_Click(object sender, EventArgs e)
        {
            ...
            string Custname = textBoxCustomerName.Text;
            int id = GetCustId(Custname);
            if (id == 0 && (Convert.ToDouble(textBoxTotalWithVat.Text) > Convert.ToDouble(textBoxReceiveAmount.Text)))
            {
        ...
        private void Insert_Customer_Ledger(int id)
        {
            ...
            if(Convert.ToDouble(textBoxDueAmount.Text) == 0.0)
            {
                ledger_credit= Convert.ToDouble(textBoxNetTotal.Text);
            }
            double ledger_debit = Convert.ToDouble(textBoxNetTotal.Text);
            string ledger_remarks = "";
            int zero = 0;
            int Adjustment = 0;
            string conStr = ConfigurationManager.ConnectionStrings["PosConString"].ToString();
            SqlConnection connection = new SqlConnection(conStr);
            string query11 = "INSERT INTO CustomerLedger(ReceiveDate,CustomerID,InvoiceNo,Debit,Credit,Adjustment,Remarks,NextPaymentDate,IsPreviousDue) VALUES('" + date + "','" + id + "','" + ledger_invoice_no + "','" + ledger_debit + "','" + ledger_credit + "','"+ Adjustment + "','" + ledger_remarks + "','" + dateTimePicker1.Text + "','"+ zero + "')";
        ...
        private void dataGridViewSales_DoubleClick_1(object sender, EventArgs e)
        {
            ...
            textBoxPrice.Text = dataGridViewSales.SelectedRows[0].Cells[2].Value.ToString();
        ...
        private void buttonUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                textBoxQuantity.Text = textBoxProductName.Text = textBoxProCate.Text = textBoxPrice.Text = textBoxPdoductCode.Text = textBoxUnitType.Text = "";
        ...
        private void buttonDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //DataGrid();
                SeeTotal_Component();
                textBoxProCate.Text = textBoxProductName.Text = textBoxPrice.Text = textBoxDiscountTaka.Text = textBoxQuantity.Text = textBoxUnitType.Text = textBoxPdoductCode.Text = "";
            }
            catch (Exception)
            {
                MessageBox.Show("Please Select a Product in DataGrid...");
        ...
        {
            if (textBoxReceiveAmount.Text == "")
            {
                textBoxReceiveAmount.Text = "0";
                if (Convert.ToInt32(textBoxReceiveAmount.Text) >= Convert.ToInt32(textBoxNetTotal.Text))
                {
                    textBoxReturnAmount.Text = (Convert.ToInt32(textBoxReceiveAmount.Text) - Convert.ToInt32(textBoxNetTotal.Text)).ToString();
                    textBoxDueAmount.Text = "0";
                }
                if (Convert.ToInt32(textBoxReceiveAmount.Text) < Convert.ToInt32(textBoxNetTotal.Text))
                {
                    textBoxDueAmount.Text = (Convert.ToInt32(textBoxNetTotal.Text) - Convert.ToInt32(textBoxReceiveAmount.Text)).ToString();
                    textBoxReturnAmount.Text = (Convert.ToInt32(textBoxReceiveAmount.Text) - Convert.ToInt32(textBoxNetTotal.Text)).ToString();
                }
        ...
        {
            if (textBoxTotalVat.Text != "" || textBoxTotalWithVat.Text != "" || textBoxReceiveAmount.Text != "")
            {
                string Custname = textBoxCustomerName.Text;
                int id = GetCustId(Custname);
